...
OUTPUT_DIR = "generated_pdfs"  # Directory to store generated PDFs
...
def create_cover_page(filename, user_details):
    """Create a temporary PDF overlay with user details to write over the original."""
...
def overlay_pdf(original_pdf_filename, overlay_pdf_filename, output_pdf_filename):
...
def create_content_pdf(filename, title, subheadings, content_json):
...
def merge_pdfs(cover_page_path, content_pdf_path, output_path):
...
async def generate_case_study(job_id: str, request: CaseStudyRequest):
    ...
    try:
        ...
        # Set up paths
        temp_dir = os.path.join(OUTPUT_DIR, job_id)
        os.makedirs(temp_dir, exist_ok=True)
        
        cover_page_filename = os.path.join(temp_dir, "cover_page_temp.pdf")
        output_file_front = os.path.join(temp_dir, "out_page.pdf")
        temp_pdf_filename = os.path.join(temp_dir, "temp_content.pdf")
        ...
        # Create cover page
        create_cover_page(cover_page_filename, user_details_dict)
        overlay_pdf(FIRSTPAGE_PDF, cover_page_filename, output_file_front)
        
        # Create content PDF
        create_content_pdf(temp_pdf_filename, request.title, request.subheadings, content_json)
        
        # Merge PDFs
        success = merge_pdfs(output_file_front, temp_pdf_filename, final_pdf_path)
        ...
        try:
            os.remove(cover_page_filename)
            os.remove(output_file_front)
...
async def download_pdf(job_id: str):
    ...
    if "file_name" not in job:
        raise HTTPException(status_code=500, detail="File name not found in job data")
